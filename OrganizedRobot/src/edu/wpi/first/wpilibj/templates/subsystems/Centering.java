...
//            turnStop();
//        }
//        if(centering && (Timer.getFPGATimestamp()-startCenterTime)>=5){
//            centering = false;
//            return distance;
//        }
//            
...
//        return 0;
    ...
    //        leftJoy = new Joystick(1);
            ...
            cc.addCriteria(NIVision.MeasurementType.IMAQ_MT_BOUNDING_RECT_HEIGHT, 50, 300, false);
            cc.addCriteria(NIVision.MeasurementType.IMAQ_MT_BOUNDING_RECT_WIDTH, 40, 300, false);
        }
        ...
        */
        public int getOffset(ParticleAnalysisReport r){
            return Math.abs(320-r.center_mass_x);
        }
        public int leftOrRight(ParticleAnalysisReport r){
            if(r.center_mass_x>320)
                return 1;
            return -1;
        }
        public ParticleAnalysisReport getReport(ColorImage image){
            try{
                    goal = 0;
                    before = image;
                    thresh = image.thresholdRGB(70, 170, 170, 255, 200, 255);
                    ...
                    for(int i = 0; i<rep.length; i++){
                        if(rep[i].center_mass_x>goal){
                            p = i;
                            r = rep[i];
                            goal = (rep[i].center_mass_x);
